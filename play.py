...
def draw_info(file_, img):
    ...
    draw.text((img.width // 2, img.height // 8),
              tag.title if tag.title is not None else remove_extension(file_),
              '#ffffff', title_font)
...
def openfile(file_: str) -> None:
    ...
    p = subprocess.Popen(
        ...
        '0:v -map 1:a -c:v rawvideo -c:a pcm_s16le -t {5} -f avi - | ffplay -'
